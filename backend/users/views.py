...
class UserViewSet(mixins.ListModelMixin,
    ...
    @action(detail=False, methods=['post'],
            permission_classes=[
                permissions.IsAuthenticated], url_path='set_password'
            )
    ...
    @action(detail=False, methods=['get'],
            permission_classes=[
                permissions.IsAuthenticated], url_path='subscriptions'
            )
    ...
    @action(detail=True, methods=['post', 'delete'],
            permission_classes=[
                permissions.IsAuthenticated], url_path='subscribe'
            )
...
class AuthView(viewsets.ViewSet):
    ...
    def login(self, request):
        ...
        if not user:
            return Response({'non_field_errors': ['Неверные учетные данные.']},
                            status=status.HTTP_400_BAD_REQUEST
                            )
